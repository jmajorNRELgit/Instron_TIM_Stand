...
daq = DAQ.DAQ()
daq.set_specific_channels([2,3,4,5,6,7])
    ...
    def __init__(self):
        ...
        with open('DAQ_data.txt', 'a') as f:
            if os.stat('DAQ_data.txt').st_size == 0: #if the data file is empty this adds the headers
                f.write('Time(s),STD,Two,Three,Four,Five,Six,Seven\n')
        ...
        self.chan2_fit = np.poly1d(list(coefficients['Channel 2:']))
        self.chan3_fit = np.poly1d(list(coefficients['Channel 3:']))
        self.chan4_fit = np.poly1d(list(coefficients['Channel 4:']))
        self.chan5_fit = np.poly1d(list(coefficients['Channel 5:']))
        self.chan6_fit = np.poly1d(list(coefficients['Channel 6:']))
        self.chan7_fit = np.poly1d(list(coefficients['Channel 7:']))

        self.fit_list = (self.chan2_fit, self.chan3_fit, self.chan4_fit, self.chan5_fit, self.chan6_fit, self.chan7_fit)
...
def animate(i):
    ...
    y1List = list(df['Two'])
    y2List = list(df['Three'])
    y3List = list(df['Four'])
    y4List = list(df['Five'])
    y5List = list(df['Six'])
    y6List = list(df['Seven'])

    STD = list(df['STD'])

    if len(y1List) > 0:

        highest_temperature = [y1List[-1],y2List[-1],y3List[-1],y4List[-1],y5List[-1],y6List[-1]]
    ...
    if app.zoom1 == 0:
        app.ax1.plot(xList,y1List, label='Chan 2')
        app.ax1.plot(xList,y2List, label='Chan 3')
        app.ax1.plot(xList,y3List, label='Chan 4')
        app.ax1.plot(xList,y4List, label='Chan 5')
        app.ax1.plot(xList,y5List, label='Chan 6')
        app.ax1.plot(xList,y6List, label='Chan 7')
        app.ax1.set_title("Temp plot")

    else:
        app.ax1.plot(xList[-c:],y1List[-c:], label='Chan 2')
        app.ax1.plot(xList[-c:],y2List[-c:], label='Chan 3')
        app.ax1.plot(xList[-c:],y3List[-c:], label='Chan 4')
        app.ax1.plot(xList[-c:],y4List[-c:], label='Chan 5')
        app.ax1.plot(xList[-c:],y5List[-c:], label='Chan 6')
        app.ax1.plot(xList[-c:],y6List[-c:], label='Chan 7')
    ...
    if len(y1List) >= c:
        app.ax1.plot(xList[-c:], [i +3 for i in misc.current_maximum_temperature[-c:]],label='Saved data') #shows the zoom1 of the data being analized for standard deviation
    else:
...
class TIM(tk.Tk):
    ...
    def workerThread1(self):
        ...
        while stop == 0:
           ...
           standard_deviation = np.mean([np.std(self.temp_list[-20:]),np.std(self.temp_list[-20:]),np.std(self.temp_list[-20:]),np.std(self.temp_list[-20:])])
           ...
           misc.DAQ_data.append(temp_data)
